...
public class StreamConsumerServiceImpl implements StreamConsumerService {
    ...
    // private final CruisePlannerStreamService cruisePlannerProducerService;
    ...
    public StreamConsumerServiceImpl(final Materializer materializr,
        ...
        // this.cruisePlannerProducerService = cruisePlannerProducerService;
    ...
    public static void startKafkaStream(final String[] args) throws Exception {
        ...
        streamsConfiguration.put(StreamsConfig.BOOTSTRAP_SERVERS_CONFIG,
            bootstrapServers);
        // Specify default (de)serializers for record keys & for record values.
        ...
        // final KStream<byte[], String> uppercasedWithMap = textLines.map((key,
        // value) -> new KeyValue<>(key, value.toUpperCase()));
        ...
        // debatable and depends on your use case. If in doubt, don't do it.
        final KStream<String, String> originalAndUppercased =
            textLines.map((key, value) -> KeyValue.pair(value,
                value.toUpperCase()));
        ...
        // because the default serializers (cf. streaming configuration) do
        // not match the type of this particular KStream instance.
        ...
        // the processing topology. We opt for this unconditional call here
        // because this will make it easier for you to play around with the
        // example when resetting the application for doing a re-run (via the
        ...
        // must rebuilt its local state from scratch, which will take time
        // and will require reading all the state-relevant data from the
        // Kafka cluster over the network. Thus in a production scenario you
        // typically do not want to clean up always as we do here but rather
        // only when it is truly needed, i.e., only under certain conditions
        // (e.g., the presence of a command line flag for your app).
        // See `ApplicationResetExample.java` for a production-like example.
        streams.cleanUp();
        streams.start();

        // Add shutdown hook to respond to SIGTERM and gracefully
        // close Kafka Streams
